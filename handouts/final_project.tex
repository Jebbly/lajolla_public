...
Even if you want to do research in this space, starting from implementing existing work in lajolla is a great idea, since it will teach you a lot about what has been done and what are their problems.
...
We will have a checkpoint at 3/9: send us a brief progress report in a few pages on Canvas (ideally provide results and images) describing what you did and what you plan to do next.

\section{Implementation ideas}

Following are projects that implement papers we talked about in the class (some of them are from previous classes). As mentioned, starting from implementing existing work is a great idea for research, since you will know the problems of existing approaches much better.
I did not sort the projects in terms of difficulty. It is possible that you can combine multiple of below.
...
\paragraph{Glints rendering}
Implement Yan et al.'s normal map filtering technique~\cite{Yan:2014:RGH} or Jakob's discrete stochastic microfacet models~\cite{Jakob:2014:DSM} in lajolla. As a bonus, implement Wu's mesh-based data structure~\cite{Wu:2025:PME} for faster rendering.

\paragraph{Tesselation-free displacement mapping.}
Implement an on-the-fly tesselation-free displacement mapping~\cite{Thonat:2021:TDM,Hoetzlein:2025:PDM} in lajolla.
...
Implement Kettunen et al.'s unbiased ray marching estimator~\cite{Kettunen:2021:URT} or Peters' biased Jackknife estimator~\cite{Peters:2025:JTM}.
...
\paragraph{GPIS.}
Implement Seyb's Gaussian process model for rendering stochastic geometry~\cite{Seyb:2024:MPM} (or the ``practical'' version from Xu~\cite{Xu:2025:PGP}, not sure which one is easier) in lajolla.
...
\paragraph{Vector-valued ratio control variates}
Implement Lu et al.'s ratio control variates~\cite{Lu:2025:VMC} in lajolla. You can apply it to either BSDF sampling, light sampling, or participating media sampling.
...
\paragraph{Manifold exploration.} Implement Zeltner's specular manifold sampling~\cite{Zeltner:2020:SMS} or Fan's specular polynomial~\cite{Fan:2024:SP} in lajolla. Start with a basic LSDE path with reflective objects (instead of refraction).
...
Implement West's stylized path tracing algorithm~\cite{West:2024:SRF} for non-photorealistic rendering. 

\paragraph{Glow discharge.}
Implement Sivaram's Glow Discharge paper~\cite{Edavamadathil:2025:MRG}.

\paragraph{Gaussian surfels differentiable radiosity.}
Implement Jiang's differentiable radiosity work~\cite{Jiang:2025:DLT}.

\paragraph{Gaussian integral operator.}
Implement Lu's Gaussian integral operator for precomputed radiance transfer~\cite{Lu:2025:GIL}.

\paragraph{Bilateral gradient filtering.}
Implement Chang-Yang-Belhe's gradiant filtering algorithm for inverse rendering~\cite{Chang:2024:SBG}. You can try it on a simple texture recovery task.

\paragraph{Residual path rendering.}
Implement Xu's residual path rendering algorithm~\cite{Xu:2024:RPI}. Implementing a subset of the sampling techniques is sufficient.
...
Focus on the rendering technology. Analyze the technology you used in the game and why you used them. Compare to alternatives.
...
\paragraph{Differentiable stylized rendering.}
Can we make the stylized path tracing algorithm from West~\cite{West:2024:SRF} differentiable and learn the stylization function from data or loss functions? What would be the computational challenge of computing the derivatives, how do we parameterize the stylization functions, and how do we collect data or setup loss functions?

\paragraph{Differentiable Doppler time-of-flight rendering}
Doppler time-of-flight imaging~\cite{Heide:2015:DTI} is an imaging device that uses the velocity of the objects in motion to capture highly detailed motion. The simulation of the device: Doppler time-of-flight rendering~\cite{Kim:2023:DTR} has been developed recently. Can we differentiate the simulation for better reconstruction of the 3D scene in motion? As shown by Kim~\cite{Kim:2023:DTR}, there are likely many interesting computational challenges that will show up when we differentiate with the system. In particular the variance of the derivative estimator will likely be very high.

\paragraph{Differentiable refractive radiative transfer equation solver}
Refractive radiative transfer equation~\cite{Pediredla:2020:PTE} describes a light transport scenario where the rays can ``curve'' due to the continuous change of index of refraction of the medium. Efficient path tracing solvers have been proposed by Pediredla, but we do not know yet how to efficiently compute the derivatives of refractive radiative transfer equation solvers. How do we save the memory akin the path replay backpropagation, how do we reduce the variance, and how do we resolve discontinuities?

\paragraph{Differentiable cone-based physical light transport for wave optics}
Steinberg~\cite{Steinberg:2025:WTG} recently proposed a wave optics simulator based on cone tracing (instead of ray tracing) that has similar structure to a (bidirectional) path tracer. Can we make their model differentiable? What are the computational challenges? What are the applications?
...
\paragraph{Crofton formula for edge sampling.}
\href{https://en.wikipedia.org/wiki/Crofton_formula}{Crofton formula} from integral geometry relates an integral over a curve to an integral over all lines that intersect with the curve. Thus we can use it for designing algorithms to sample points on the curve, which makes it very suitable for an edge sampling algorithm. We explored a (biased but consistent) version of this type of algorithms in our recent work~\cite{Belhe:2025:ASD}, but did not apply it to path tracing problems. Can we use similar algorithms for edge sampling and how would it fair against alternatives like projective sampling?

\paragraph{Variance analysis and combination of attached/detached samplers in differentiable rendering.}
Zeltner~\cite{Zeltner:2021:MCE} showed that to importance sample the derivative of an integral, we can either first apply the differentiation, then do the importance sampling (they call it ``detached'' sampling). Or we can first apply the importance sampling, then do the differentiation (they call it ``attached'' sampling). The paper didn't answer the question of when is one better than the other: can we theoretically analyze the variance of the two estimators? Can we combine the two estimators, similar to multiple importance sampling, so that the better one is used whenever we need them? For this project, you do not need to implement the esimators inside a renderer. Analyzing them in some synthetic functions you want to integrate and differentiate should be good enough.

\paragraph{Differentiable Gaussian process stochastic geometry light transport.}
Can we make the Gaussian process stochastic geometry light transport models~\cite{Seyb:2024:MPM,Xu:2025:PGP} directly differentiable? How do we use it for inverse rendering?

\paragraph{Stochastic geometry beyond Gaussian processes.}
Seyb used Gaussian processes~\cite{Seyb:2024:MPM} to model stochastic geometry, but there could be other choices such as Student-t processes or Levy processes. What are the difference in the appearance between these processes and when should we choose these? (This idea was partly suggested by ChatGPT.)

\paragraph{Better/generalized gradient filtering.}
In our recent work~\cite{Chang:2024:SBG} we show that a simple cross-bilateral filter over the gradients can significantly improve the inverse recovery of textures, volumes, and meshes. Can we design other filters that work better? (For example, how about a non-local means filter? How about guided image filter?) Should a good preconditioning filter be different from a good denoising filter? Can we design filters that work for other modalities, such as point clouds, Gaussian splats, SVBRDFs, or even neural networks?
...
Can we design a path guiding algorithm~\cite{Lafortune:1995:5RV,Muller:2017:PPG} that builds a 5D data structure to decide which point and direction the path should start with.
...
\paragraph{Improved residual path rendering for animation.}
Our recent residual path rendering paper~\cite{Xu:2024:RPI} shows that it is possible to directly compute the difference of two frames using control variates and derive importance sampling algorithms to avoid sampling paths where things have not changed between frames. The paper only shows applications to re-rendering: what would be the practical concerns when we apply the method to animation? The ``path mapping'' we used in the paper also only works for affine transformation of the animated object. Can we derive the path mapping for general deformation?
...
We can explore ways to update the path guiding data structures over time.
...
\paragraph{Analytical precomputed rendering.}
Our recent work~\cite{Lu:2025:GIL} uses Gaussians to fit an integral operator that results in analytical solutions that can be used for precomputed rendering. Are there other functions that have similar properties (e.g., polynomials or spherical Gaussians), and can we use them for similar tasks? Can we collect a class of functions and choose the best subsets for our tasks during fitting? In particular, infinite support Gaussians can have issues when dealing with finite domain boundaries which introduce discontinuities.
...
We can investigate the use of neural networks for designing mutation in a Metropolis light transport renderer.
...
I would also recommend to implement this in \href{https://developer.nvidia.com/blog/differentiable-slang-example-applications/}{Slang} to use its automatic differentiation feature and GPU backends.
...
\paragraph{HotSpot for inverse rendering.}
Our recent work proposed a loss based on heat transfer to optimize neural signed distance fields~\cite{Wang:2025:HSD}. In the paper we only show results on fitting the geometry to a point cloud. Can we do the same thing using a rendering loss~\cite{Bangaru:2022:DRN}? Will this work better than the alternatives such as the eikonal loss?

\paragraph{Animated BRDFs and phase functions.}
How would BRDFs and phase functions change over time~\cite{Sun:2007:TVB}? For example, when we deform a surface, how would the microfacet statistics on the surface change~\cite{Atanasov:2022:MT}? If we have a fluid simulator of smoke, can we also simulate how the phase function of smoke changes over time? What is the underlying physics? A potential starting point is to use the data collected by Sun et al.~\cite{Sun:2007:TVB} and fit a physical model to the data. You can also simply propose your own model and we can worry about fitting later.
...
As usual, I would also recommend to implement this in \href{https://developer.nvidia.com/blog/differentiable-slang-example-applications/}{Slang} to use its automatic differentiation feature and GPU backends.
...
\paragraph{Fast ray tracing for Neural Geometry Fields.}
Our recent work on Neural Geometry Fields~\cite{Edavamadathil:2024:NGF} proposed a data structure for storing geometric displacement using neural networks. However, we only explored rasterization for NGF rendering in the paper. How do we efficiently ray trace a NGF on-the-fly without tesselation~\cite{Thonat:2021:TDM}? Would the best rendering algorithm be different from the existing ones for displacement mapping?
...
\paragraph{Abusing RT cores.}
RT cores have been recently used for non-ray-tracing tasks such as nearest neighbor search~\cite{Zhu:2022:RAN}, database query~\cite{Shi:2024:RBD}, and clustering~\cite{Nagarajan:2023:RAD}. Can you think of other applications of RT cores that are not exactly ray tracing? A few random ideas: generalized winding number calculation~\cite{Barill:2018:FWN}, fast smoothed distance computation~\cite{Madan:2022:FES}, 2D vector graphics rendering, and attention mechanism.

\paragraph{Fixed-point or float16 rendering.}
Rendering is typically done in 32-bit or 64-bit floating point. What if we do it using fixed-point? (Certain mobile hardware may require so.) What if we do it using lower precision like float16 or even float8? (Modern GPUs now support float16 or float8 computation.) Do we need to change anything in the rendering pipelines to accomodate? Do we need adaptive precision?

\paragraph{Theorem provers for inverse rendering.}
There exist theorem provers such as \href{https://en.wikipedia.org/wiki/Z3_Theorem_Prover}{Z3} or \href{https://dreal.github.io/}{dReal} that can relatively efficiently solve problems of Satisfiability Modulo Theories (a generalized version of the boolean satisfiability problem). Since SMT/satisfiability can be seen as an inverse problem (finding variables that will satisfy an equation), can we somehow use these tools for inverse rendering? To achieve this, we need to implement a renderer in either Z3 or dReal, and then use these tools to find unknown scene parameters that will render to target images. How effective are these tools and can we combine~\cite{Inala:2018:RCN} them with existing differentiable renderers?

\paragraph{Relationship between differentiable rendering and reinforcement learning.}
Dahm~\cite{Dahm:2017:LLT} has shown that there is a strong relationship between rendering equation and Bellman equation -- the fundamental equation in reinforcement learning: both equations are a recursive \href{https://en.wikipedia.org/wiki/Fredholm_integral_equation#Equation_of_the_second_kind}{Fredholm Integral Equation of the Second Kind}. To optimize the Bellman equation, we often have to compute its gradients, aka policy gradients. The computation of policy gradients is thus strongly related to the task of differentiable rendering. Can we borrow techniques from both fields and benefit each other? For example, can we take differentiable rendering algorithms and apply them in reinforcement learning? Or, can we take reinforcement learning algorithms and apply them for differentiable/inverse rendering?
